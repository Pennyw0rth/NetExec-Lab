...
$UserName = "gMSA-obelix$"  # User account to grant permissions to
$DomainController = "village.armorique.local"  # Domain Controller FQDN
$DomainDistinguishedName = (Get-ADDomain -Server $DomainController).DistinguishedName  # Get the Domain DN

# Retrieve the User SID
$objGroup = Get-ADServiceAccount -Identity $UserName -Server $DomainController
